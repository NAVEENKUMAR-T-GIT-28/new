...
class EmailWatchTimeDigestResponse {
  @IsString()
  @JSONSchema({ description: 'Summary message' })
  message!: string;

  @IsInt()
  @Min(0)
  @JSONSchema({ description: 'Emails sent successfully' })
  sent!: number;

  @IsInt()
  @Min(0)
  @JSONSchema({ description: 'Send failures' })
  failed!: number;

  @IsInt()
  @Min(0)
  @JSONSchema({ description: 'Skipped (missing user or email)' })
  skippedNoUser!: number;

  @IsInt()
  @Min(0)
  @JSONSchema({ description: 'Learners with watch-time rows' })
...
class UserAccumulatedWatchTimeRow {
  @IsString()
  @JSONSchema({ description: 'User ID' })
  userId!: string;

  @IsString()
  @JSONSchema({ description: 'Learner email' })
  email!: string;

  @IsString()
  @JSONSchema({ description: 'Learner full name' })
  name!: string;

  @IsNumber()
  @JSONSchema({ description: 'Accumulated watch time in seconds', example: 7200 })
  totalSeconds!: number;

  @IsNumber()
  @JSONSchema({ description: 'Accumulated watch time in hours', example: 2 })
  totalHours!: number;

  @IsNumber()
  @JSONSchema({ description: 'Accumulated watch time in hours (2 decimals)', example: 2.13 })
  totalHoursRounded!: number;

  @IsString()
  @JSONSchema({ description: 'Readable duration', example: '2 hours 8 minutes' })
  readableDuration!: string;
}

class CourseVersionUserAccumulatedWatchTimeResponse {
  @IsString()
  @JSONSchema({ description: 'Success message' })
  message!: string;

  @IsArray()
  @Type(() => UserAccumulatedWatchTimeRow)
  @JSONSchema({ description: 'Per-user accumulated watch-time rows' })
  users!: UserAccumulatedWatchTimeRow[];
}
