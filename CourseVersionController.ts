...
- Manager of the course.`,
  ...
  @OpenAPI({
    summary: 'Get accumulated watch time per user for a course version',
    description: `Returns accumulated watch time for each learner in a course version.<br/>
Accessible to:
- Instructor or manager for the course version.`,
  })
  @Authorized()
  @Get('/:courseId/versions/:versionId/watch-time/users')
  @ResponseSchema(CourseVersionUserAccumulatedWatchTimeResponse, {
    description: 'Per-user watch time fetched successfully',
  })
  @ResponseSchema(BadRequestErrorResponse, {
    description: 'Bad Request Error',
    statusCode: 400,
  ...
  async getCourseVersionUserWatchTime(
    @Params() params: GetCourseVersionWatchTimeParams,
    ...
    if (!ability.can(CourseVersionActions.Modify, courseVersionSubject)) {
      throw new ForbiddenError(
        'You do not have permission to view learner watch time for this course version',
      );
    }

    return this.courseVersionService.getCourseVersionAccumulatedWatchTimePerUser(
      courseId,
      versionId,
    );
  }
  ...
  @OpenAPI({
    summary: 'Email each learner their watch time (hours) for this course version',
    description: `Sends one email per learner who has watch-time data, using each user's registered email.<br/>
Accessible to:
- Instructor or manager who can modify this course version.`,
  })
  @Authorized()
  @Post('/:courseId/versions/:versionId/email-watch-time-to-learners')
  @HttpCode(200)
  @ResponseSchema(EmailWatchTimeDigestResponse, {
    description: 'Digest emails queued/sent',
  })
  @ResponseSchema(BadRequestErrorResponse, {
    description: 'Bad Request Error',
    statusCode: 400,
  ...
  async emailLearnersWatchTimeDigest(
    @Params() params: GetCourseVersionWatchTimeParams,
    @Ability(getCourseVersionAbility) { ability },
  ): Promise<EmailWatchTimeDigestResponse> {
    const { courseId, versionId } = params;
    ...
    if (!ability.can(CourseVersionActions.Modify, courseVersionSubject)) {
      throw new ForbiddenError(
        'You do not have permission to send learner watch time emails for this course version',
      );
    }

    return this.courseVersionService.emailLearnersTheirWatchTimeSummary(
      courseId,
      versionId,
    );
  }
