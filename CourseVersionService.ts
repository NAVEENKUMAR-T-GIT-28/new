...
import {
  CohortsResponse,
  CourseVersionUserAccumulatedWatchTimeResponse,
...
export class CourseVersionService extends BaseService {
  ...
  async getCourseVersionAccumulatedWatchTimePerUser(
    courseId: string,
    versionId: string,
  ): Promise<CourseVersionUserAccumulatedWatchTimeResponse> {
    const averageVideoDurationSeconds =
      await this.getAverageCourseVideoDurationSeconds(versionId);
    const maxSecondsPerView = this.getVideoWatchCapSeconds(
      averageVideoDurationSeconds,
    ...
    const perUserRows =
      await this.progressRepository.getWatchTimeSecondsByUserForCourseVersion(
        courseId,
        versionId,
        maxSecondsPerView,
      );
    ...
    const users = await Promise.all(
      perUserRows.map(async row => {
        const user = row.userId ? await this.userRepo.findById(row.userId) : null;
        ...
        return {
          ...
          readableDuration: this.formatWatchTimeReadable(row.totalSeconds),
        };
      }),
    );

    return {
      message: `Per-user watch time fetched successfully for course "${course.name}" version "${courseVersion.version}".`,
      users: users.sort((a, b) => b.totalSeconds - a.totalSeconds),
    };
  }

  /**
   * Emails each learner their own capped watch-time summary for this course version.
   */
  async emailLearnersTheirWatchTimeSummary(
    courseId: string,
    versionId: string,
  ): Promise<{
    message: string;
    ...
    const perUser = await this.progressRepository.getWatchTimeSecondsByUserForCourseVersion(
      courseId,
      versionId,
      maxSecondsPerView,
    );
    ...
    for (const row of perUser) {
      ...
      try {
        ...
        const readable = this.formatWatchTimeReadable(row.totalSeconds);
        const hoursRounded = Number((row.totalSeconds / 3600).toFixed(2));

        await this.mailService.sendMail({
          to: user.email,
          subject: `Your learning time — ${course.name} (${courseVersion.version})`,
          text:
            `Dear ${user.firstName},\n\n` +
            `Here is your recorded time on the platform for the course "${course.name}", version "${courseVersion.version}":\n\n` +
            `• Total time: ${readable} (${hoursRounded} hours)\n\n` +
            `This summary uses the same rules as course analytics (individual viewing sessions are capped for fairness).\n\n` +
            `— ViBe\n` +
    ...
    return {
      message: `Processed ${perUser.length} learner(s) with watch data.`,
      sent,
      failed,
      skippedNoUser,
      totalLearners: perUser.length,
    };
  }

  private formatWatchTimeReadable(totalSeconds: number): string {
