...
import { useAddQuestionBankToQuiz, useAddQuestionToBank, useCourseVersionAccumulatedWatchTime, useCreateQuestion, useCreateQuestionBank, useOverallVideoAnalytics, userParseCSVtoItems, useUpdateItemOptional, useVideoUserAnalytics } from '@/hooks/hooks';
...
function TeacherCourseContent() {
  ...
  const {
    data: accumulatedProgressData,
    isLoading: isAccumulatedProgressLoading,
  } = useCourseVersionAccumulatedWatchTime(courseId || '', versionId || '');
                        ...
                        <Tabs value={videoTab} onValueChange={setVideoTab} className=" mb-4">
                          ...
                          {videoTab === "analytics" && (
                            <div className="mt-4">
                              ...
                              <div className="mt-6">
                                <CourseAccumulatedProgressTable
                                  users={accumulatedProgressData?.users || []}
                                  isLoading={isAccumulatedProgressLoading}
                                />
                              </div>
                            </div>
...
function CourseAccumulatedProgressTable({
  users,
  isLoading,
}: {
  users: AccumulatedProgressUser[];
  isLoading?: boolean;
}) {
  return (
    <Card className="bg-muted/20">
      <CardHeader className="pb-3">
        <CardTitle className="text-base">Accumulated Progress by User</CardTitle>
        <CardDescription className="text-xs">
          Total watch-time accumulation for this course version.
        </CardDescription>
      </CardHeader>
      <CardContent>
        <div className="overflow-x-auto">
          <Table>
            <TableHeader>
              <TableRow className="border-border bg-muted/30">
                <TableHead className="pl-6 w-[320px]">Student</TableHead>
                <TableHead className="w-[160px] text-left">Hours</TableHead>
                <TableHead className="w-[220px] text-left">Duration</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {isLoading && (
              ...
              {!isLoading && users.length === 0 && (
                <TableRow>
                  <TableCell colSpan={3} className="py-10 text-center text-muted-foreground">
                    No accumulated progress found.
                  </TableCell>
                </TableRow>
              )}
              {!isLoading &&
                users.map(user => (
                  <TableRow key={user.userId} className="border-b border-border/60 hover:bg-muted/30">
                    <TableCell className="pl-6">
                      <div className="flex flex-col">
                        <span className="font-medium text-foreground leading-tight">
                    ...
                    <TableCell className="text-left font-medium tabular-nums">
                      {user.totalHoursRounded.toFixed(2)}
                    </TableCell>
                    <TableCell className="text-left">{user.readableDuration}</TableCell>
                  </TableRow>
                ))}
            </TableBody>
          </Table>
