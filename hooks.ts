...
export interface CourseVersionAccumulatedWatchTimeUser {
  userId: string;
  email: string;
  name: string;
  totalSeconds: number;
...
export interface CourseVersionAccumulatedWatchTimeResponse {
  message: string;
  users: CourseVersionAccumulatedWatchTimeUser[];
}

export function useCourseVersionAccumulatedWatchTime(
  courseId: string,
  versionId: string,
): {
  data: CourseVersionAccumulatedWatchTimeResponse | undefined,
  isLoading: boolean,
  error: string | null,
  refetch: () => void
} {
  const result = api.useQuery(
    "get",
    "/courses/{courseId}/versions/{versionId}/watch-time/users" as any,
    {
      params: {
        path: {
          courseId,
    ...
    { enabled: !!courseId && !!versionId } as any,
  );

  return {
    data: result.data as CourseVersionAccumulatedWatchTimeResponse | undefined,
    isLoading: result.isLoading,
    error: result.error
      ? (result.error.message || 'Failed to fetch accumulated watch time')
      : null,
    refetch: result.refetch,
  };
}
